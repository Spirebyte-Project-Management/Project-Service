...
namespace Spirebyte.Services.Projects.Application.Commands.Handlers
{
    internal sealed class UpdatePermissionSchemeHandler : ICommandHandler<UpdatePermissionScheme>
    {
        ...
        public UpdatePermissionSchemeHandler(IPermissionSchemeRepository permissionSchemeRepository, IProjectRepository projectRepository,
        ...
        public async Task HandleAsync(UpdatePermissionScheme command)
        {
            ...
            if (!await _permissionService.HasPermission(projectPermissionScheme.ProjectId, _appContext.Identity.Id, ProjectPermissionKeys.AdministerProject))
            {
                throw new ActionNotAllowedException();
            }

            var updatedProjectPermissionScheme = new PermissionScheme(projectPermissionScheme.Id, projectPermissionScheme.ProjectId, command.Name, command.Description, command.Permissions);
