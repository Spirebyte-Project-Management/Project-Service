...
namespace Spirebyte.Services.Projects.Application.Commands.Handlers
{
    internal sealed class DeleteProjectGroupHandler : ICommandHandler<DeleteProjectGroup>
    {
        ...
        public DeleteProjectGroupHandler(IProjectGroupRepository projectGroupRepository, IMessageBroker messageBroker, IPermissionService permissionService, IAppContext appContext)
        {
        ...
        public async Task HandleAsync(DeleteProjectGroup command)
        {
            ...
            if (!await _permissionService.HasPermission(projectGroup.ProjectId, _appContext.Identity.Id, ProjectPermissionKeys.AdministerProject))
            {
